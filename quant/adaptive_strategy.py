"""
自适应策略参数调整模块
实现基于市场状态（7类）的自适应参数调整系统
"""
from dataclasses import replace
from typing import Dict

from quant.logger import logger
from quant.strategy_params import StrategyParams


def get_dynamic_params_enhanced(
    base_params: StrategyParams,
    market_state: str
) -> StrategyParams:
    """
    增强版自适应参数调整系统（支持 7 类市场状态）

    参数调整策略：
    1. strong_bull: 激进策略 - 高仓位，宽松的 AI 过滤，快速止盈
    2. bull_momentum: 超激进策略 - 最高仓位，快速止盈，动态加仓
    3. weak_bull: 稳健策略 - 中高仓位，适度 AI 过滤
    4. sideways: 保守策略 - 标准仓位，严格的 AI 过滤，注重风险控制
    5. weak_bear: 防御策略 - 中低仓位，严格的 AI 过滤
    6. bear_momentum: 退守策略 - 低仓位，极度严格的 AI 过滤
    7. strong_bear: 空仓策略 - 最低仓位，禁止开仓（或仅限左侧交易）

    Args:
        base_params: 基础策略参数
        market_state: 市场状态（7 类）

    Returns:
        调整后的策略参数
    """
    try:
        params = replace(base_params)
    except Exception:
        # Fallback: try to convert to dict and recreate
        try:
            params = StrategyParams(**base_params.__dict__)
        except Exception as e:
            logger.error(f"Failed to create params copy: {e}")
            raise

    # 市场状态参数调整表（7 类）
    state_adjustments: Dict[str, Dict[str, float]] = {
        # === 牛市策略（激进）===
        'strong_bull': {
            'position_size': 0.15,  # 高仓位
            'ai_prob_threshold': 0.20,  # 宽松的 AI 过滤
            'max_hold_days': 10,  # 中等持仓周期
            'trail_atr_mult': 2.2,  # 宽松的移动止损
            'take_profit_pct': 0.08,  # 适度止盈
            'w_pullback_ma': 1.5,  # 降低左侧交易权重
            'w_vol_up': 2.5,  # 提高右侧突破权重
        },
        'bull_momentum': {
            'position_size': 0.20,  # 最高仓位
            'ai_prob_threshold': 0.15,  # 最宽松的 AI 过滤
            'max_hold_days': 5,  # 短期快速交易
            'trail_atr_mult': 2.5,  # 宽松的移动止损
            'take_profit_pct': 0.06,  # 快速止盈
            'w_pullback_ma': 1.0,  # 最低左侧交易权重
            'w_vol_up': 3.0,  # 最高右侧突破权重
            'vol_up_ratio': 1.3,  # 降低放量门槛
        },
        'weak_bull': {
            'position_size': 0.12,  # 中高仓位
            'ai_prob_threshold': 0.30,  # 适度 AI 过滤
            'max_hold_days': 12,  # 中等持仓周期
            'trail_atr_mult': 2.0,  # 适中的移动止损
            'take_profit_pct': 0.08,  # 适度止盈
            'w_pullback_ma': 2.0,  # 平衡的权重
            'w_vol_up': 2.0,  # 平衡的权重
        },
        
        # === 横盘策略（保守）===
        'sideways': {
            'position_size': 0.08,  # 标准仓位
            'ai_prob_threshold': 0.35,  # 严格的 AI 过滤
            'max_hold_days': 15,  # 较长持仓周期
            'trail_atr_mult': 1.8,  # 紧密的移动止损
            'take_profit_pct': 0.06,  # 快速止盈
            'w_pullback_ma': 2.5,  # 提高左侧交易权重（捕捉反弹）
            'w_vol_up': 1.5,  # 降低右侧突破权重
            'pullback_ma_tolerance': 1.015,  # 提高回调门槛
        },
        
        # === 熊市策略（防御）===
        'weak_bear': {
            'position_size': 0.05,  # 中低仓位
            'ai_prob_threshold': 0.45,  # 严格的 AI 过滤
            'max_hold_days': 10,  # 短期持仓周期
            'trail_atr_mult': 1.6,  # 紧密的移动止损
            'take_profit_pct': 0.05,  # 快速止盈
            'w_pullback_ma': 3.0,  # 提高左侧交易权重（捕捉反弹）
            'w_vol_up': 1.0,  # 降低右侧突破权重
            'rsi_cooled_max': 45,  # 更严格的 RSI 条件
        },
        'bear_momentum': {
            'position_size': 0.03,  # 低仓位
            'ai_prob_threshold': 0.55,  # 极度严格的 AI 过滤
            'max_hold_days': 5,  # 最短期持仓
            'trail_atr_mult': 1.4,  # 极度紧密的移动止损
            'take_profit_pct': 0.04,  # 快速止盈
            'w_pullback_ma': 4.0,  # 最高左侧交易权重
            'w_vol_up': 0.5,  # 最低右侧突破权重
            'rsi_oversold': 30,  # 更严格的超卖条件
        },
        'strong_bear': {
            'position_size': 0.02,  # 最低仓位
            'ai_prob_threshold': 0.65,  # 极度严格的 AI 过滤
            'max_hold_days': 3,  # 最短期持仓
            'trail_atr_mult': 1.2,  # 极度紧密的移动止损
            'take_profit_pct': 0.03,  # 快速止盈
            'w_pullback_ma': 5.0,  # 最高左侧交易权重
            'w_vol_up': 0.3,  # 最低右侧突破权重（几乎不交易）
            'rsi_oversold': 25,  # 极度严格的超卖条件
            'bbands_lower_bias': 0.95,  # 更严格的布林带条件
        },
    }

    # 获取当前市场状态的调整
    adjustments = state_adjustments.get(market_state, state_adjustments['sideways'])

    # 应用调整（相对于基准值的增量调整）
    for key, value in adjustments.items():
        current_value = getattr(params, key, None)
        if current_value is not None:
            if isinstance(current_value, float):
                # 对于连续参数，直接应用调整值
                setattr(params, key, value)
            elif isinstance(current_value, int):
                # 对于整数参数，应用四舍五入
                setattr(params, key, int(round(value)))

    logger.debug(f"市场状态: {market_state}, 应用调整: {adjustments}")

    return params


def get_param_transition_matrix() -> Dict[str, Dict[str, float]]:
    """
    获取参数转移矩阵
    定义不同市场状态之间的参数平滑过渡规则

    Returns:
        参数转移矩阵
    """
    # 简化的转移矩阵（实际应用中可以基于历史数据学习）
    return {
        'strong_bull': {
            'bull_momentum': 0.9,  # 强牛市 -> 牛市动量（90% 概率）
            'weak_bull': 0.1,
        },
        'bull_momentum': {
            'strong_bull': 0.7,
            'weak_bull': 0.3,
        },
        'weak_bull': {
            'strong_bull': 0.2,
            'sideways': 0.5,
            'bull_momentum': 0.3,
        },
        'sideways': {
            'weak_bull': 0.3,
            'weak_bear': 0.3,
            'strong_bull': 0.2,
            'strong_bear': 0.2,
        },
        'weak_bear': {
            'sideways': 0.5,
            'strong_bear': 0.2,
            'bear_momentum': 0.3,
        },
        'bear_momentum': {
            'weak_bear': 0.3,
            'strong_bear': 0.7,
        },
        'strong_bear': {
            'bear_momentum': 0.9,
            'weak_bear': 0.1,
        },
    }


def smooth_param_transition(
    current_params: StrategyParams,
    target_params: StrategyParams,
    smoothing_factor: float = 0.3
) -> StrategyParams:
    """
    参数平滑过渡
    在市场状态转换时，平滑过渡参数值，避免剧烈变化

    Args:
        current_params: 当前参数
        target_params: 目标参数
        smoothing_factor: 平滑因子（0-1），越小表示过渡越慢

    Returns:
        平滑过渡后的参数
    """
    try:
        params = replace(current_params)
    except Exception:
        params = StrategyParams(**current_params.__dict__)

    # 获取所有参数名称
    param_names = [f.name for f in params.__dataclass_fields__]

    # 对每个参数进行平滑过渡
    for name in param_names:
        current_value = getattr(current_params, name)
        target_value = getattr(target_params, name)

        if isinstance(current_value, (int, float)):
            # 计算平滑后的值
            smoothed_value = current_value + smoothing_factor * (target_value - current_value)
            
            # 根据类型进行转换
            if isinstance(current_value, int):
                setattr(params, name, int(round(smoothed_value)))
            else:
                setattr(params, name, smoothed_value)

    return params


# 兼容旧版函数
get_dynamic_params = get_dynamic_params_enhanced


if __name__ == "__main__":
    # 测试代码
    from quant.strategy_params import StrategyParams
    
    base_params = StrategyParams()
    
    # 测试不同市场状态的参数调整
    for state in ['strong_bull', 'bull_momentum', 'weak_bull', 'sideways', 
                   'weak_bear', 'bear_momentum', 'strong_bear']:
        adjusted_params = get_dynamic_params_enhanced(base_params, state)
        
        print(f"\n市场状态: {state}")
        print(f"  仓位大小: {base_params.position_size:.3f} -> {adjusted_params.position_size:.3f}")
        print(f"  AI 概率阈值: {base_params.ai_prob_threshold:.3f} -> {adjusted_params.ai_prob_threshold:.3f}")
        print(f"  最大持仓天数: {base_params.max_hold_days} -> {adjusted_params.max_hold_days}")
