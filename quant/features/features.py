...
from quant.infra.numba_accelerator import (
    ...
    get_numba_status,
    # Phase 9: Additional numba functions for feature optimization
    compute_rsi_numba,
    compute_macd_numba,
...
# =============================================================================
# Phase 9: Memory-efficient Feature Engineering
# =============================================================================

def _chunked_feature_calculation(df: pd.DataFrame, chunk_size: int = 10000) -> pd.DataFrame:
    """
    分块计算特征（用于大数据集）
    ...
    # 对于大数据集，分块处理
    logger.info(f"使用分块特征计算: {len(df)} 行, 块大小 {chunk_size}")
    ...
    for i in range(0, len(df), chunk_size):
        ...
        chunks.append(chunk)
        gc.collect()  # 每处理完一块后提示垃圾回收
    
    return pd.concat(chunks, ignore_index=True)


def _optimize_feature_memory(df: pd.DataFrame) -> pd.DataFrame:
    """
    优化特征列的内存使用
...
def extract_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ========== NEW FEATURES - Enhanced Feature Set ==========

    # 11. Market Percentile Features (100-day windows)
    # feat_mom_rank_100: 100-day momentum percentile
    if 'feat_pct_chg_20' in df.columns:
        df['feat_mom_rank_100'] = df['feat_pct_chg_20'].rolling(window=100, min_periods=30).apply(
            lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
        df['feat_mom_rank_100'] = df['feat_mom_rank_100'].fillna(0.5)

    # feat_vol_rank_100: 100-day volatility percentile
    if 'feat_realized_vol_20' in df.columns:
        df['feat_vol_rank_100'] = df['feat_realized_vol_20'].rolling(window=100, min_periods=30).apply(
            lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
        df['feat_vol_rank_100'] = df['feat_vol_rank_100'].fillna(0.5)

    # feat_rsi_rank_100: RSI percentile ranking
    if 'feat_rsi_val' in df.columns:
        df['feat_rsi_rank_100'] = df['feat_rsi_val'].rolling(window=100, min_periods=30).apply(
            lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
        df['feat_rsi_rank_100'] = df['feat_rsi_rank_100'].fillna(0.5)

    # 12. Money Flow Features
    # feat_money_flow: Price * Volume * direction
    direction = np.where(close_series >= df[open_col], 1, -1)
    df['feat_money_flow'] = close_series * df[vol_col] * direction

    # feat_money_flow_ma_ratio: MF / MF 20-day MA
    mf_ma_20 = df['feat_money_flow'].rolling(window=20).mean()
    df['feat_money_flow_ma_ratio'] = df['feat_money_flow'] / (mf_ma_20 + 1e-8)

    # feat_money_flow_trend: MF linear regression slope (5-day)
    mf_values = df['feat_money_flow'].values.astype(float)
    n_days = len(mf_values)
    mf_slope = np.full(n_days, np.nan)
    x = np.arange(5, dtype=float)
    x_mean = 2.0
    x_var = np.var(x)
    for i in range(4, n_days):
        y = mf_values[i-4:i+1]
        y_mean = np.mean(y)
        if y_mean != 0 and x_var > 0:
            cov = np.mean((x - x_mean) * (y - y_mean))
            slope = cov / x_var
            mf_slope[i] = slope / (abs(y_mean) + 1e-8)
    df['feat_money_flow_trend'] = mf_slope

    # 13. Volatility Regime Features
    # feat_vol_of_vol: Volatility of realized volatility (20-day rolling std of realized_vol_20)
    if 'feat_realized_vol_20' in df.columns:
        df['feat_vol_of_vol'] = df['feat_realized_vol_20'].rolling(window=20).std()

    # feat_vol_skew: Return skewness over 20 days
    daily_ret = close_series.pct_change(1)
    df['feat_vol_skew'] = daily_ret.rolling(window=20).skew()

    # feat_vol_kurt: Return kurtosis
    df['feat_vol_kurt'] = daily_ret.rolling(window=20).kurt()

    # 14. Correlation Features
    # feat_market_corr_20: 20-day correlation with market index
    market_feats = _get_market_features()
    if market_feats is not None:
        # Join with market features again for correlation calculation
        temp_df = pd.DataFrame({'close': close_series}, index=df.index)
        if isinstance(df.index, pd.DatetimeIndex):
            temp_df = temp_df.join(market_feats[['feat_market_pct_1']], how='left')
        elif 'date' in df.columns:
            temp_df['date'] = pd.to_datetime(df['date'])
            temp_df = temp_df.merge(market_feats[['feat_market_pct_1']], left_on='date', right_index=True, how='left')
        elif 'Date' in df.columns:
            temp_df['Date'] = pd.to_datetime(df['Date'])
            temp_df = temp_df.merge(market_feats[['feat_market_pct_1']], left_on='Date', right_index=True, how='left')

        if 'feat_market_pct_1' in temp_df.columns:
            stock_ret = close_series.pct_change(1)
            market_ret = temp_df['feat_market_pct_1']
            # Calculate rolling correlation
            df['feat_market_corr_20'] = stock_ret.rolling(window=20).corr(market_ret)

    # Fill NaN values for new features
    new_feature_cols = [
        'feat_mom_rank_100', 'feat_vol_rank_100', 'feat_rsi_rank_100',
        'feat_money_flow', 'feat_money_flow_ma_ratio', 'feat_money_flow_trend',
        'feat_vol_of_vol', 'feat_vol_skew', 'feat_vol_kurt', 'feat_market_corr_20'
    ]
    for col in new_feature_cols:
        if col in df.columns:
            if 'rank' in col:
                df[col] = df[col].fillna(0.5)  # Percentiles fill with median
            elif 'corr' in col:
                df[col] = df[col].fillna(0.0)  # Correlation fill with 0
            elif 'skew' in col or 'kurt' in col:
                df[col] = df[col].fillna(0.0)  # Skewness/kurtosis fill with 0
            elif 'money_flow' in col:
                df[col] = df[col].fillna(0.0)  # Money flow fill with 0
            else:
                df[col] = df[col].fillna(0.0)

    # Normalize money flow features (z-score on rolling basis)
    for col in ['feat_money_flow', 'feat_money_flow_ma_ratio']:
        if col in df.columns:
            ...
            df[f'{col}_zscore'] = df[f'{col}_zscore'].fillna(0.0)
...
def get_sector_data(sector_code: str) -> pd.DataFrame | None:
    ...
    Args:
        sector_code: 板块代码 (e.g., 's_sh.000001' for Shanghai composite)
...
def add_sector_relative_features(df: pd.DataFrame, sector_code: str) -> pd.DataFrame:
    """
    添加相对于行业/板块的特征
    ...
    """
    df = df.copy()
    sector_df = get_sector_data(sector_code)

    if sector_df is None:
        # 如果无法获取板块数据，填充默认值
        df['feat_sector_relative_mom'] = 0.0
        df['feat_sector_relative_vol'] = 0.0
        df['feat_sector_rank'] = 0.5
        df['feat_sector_corr'] = 0.0
        return df

    # 获取板块收盘价
    sector_close_col = 'close' if 'close' in sector_df.columns else 'Close'
    if sector_close_col not in sector_df.columns:
        df['feat_sector_relative_mom'] = 0.0
        df['feat_sector_relative_vol'] = 0.0
        df['feat_sector_rank'] = 0.5
        df['feat_sector_corr'] = 0.0
    ...
    # 对齐数据
    stock_close_col = 'close' if 'close' in df.columns else 'Close'
    stock_close = df[stock_close_col]
    ...
    if isinstance(temp_df.index, pd.DatetimeIndex):
        temp_df = temp_df.join(sector_df[[sector_close_col]].rename(columns={sector_close_col: 'sector_close'}), how='left')
    elif 'date' in df.columns:
        temp_df['date'] = pd.to_datetime(df['date'])
        temp_df = temp_df.merge(
            sector_df[[sector_close_col]].rename(columns={sector_close_col: 'sector_close'}),
            left_on='date',
            right_index=True,
            how='left'
        )

    if 'sector_close' not in temp_df.columns:
        df['feat_sector_relative_mom'] = 0.0
        df['feat_sector_relative_vol'] = 0.0
        df['feat_sector_rank'] = 0.5
        df['feat_sector_corr'] = 0.0
    ...
    # 计算板块收益率
    temp_df['sector_ret'] = temp_df['sector_close'].pct_change(1)

    # feat_sector_relative_mom: Momentum relative to sector average
    stock_mom_20 = stock_close.pct_change(20)
    sector_mom_20 = temp_df['sector_close'].pct_change(20)
    df['feat_sector_relative_mom'] = stock_mom_20 - sector_mom_20

    # feat_sector_relative_vol: Volatility relative to sector
    stock_vol = temp_df['stock_ret'].rolling(window=20).std()
    sector_vol = temp_df['sector_ret'].rolling(window=20).std()
    df['feat_sector_relative_vol'] = stock_vol - sector_vol

    # feat_sector_rank: Stock's performance rank within sector (using self-relative approximation)
    # Since we process one stock at a time, we calculate its relative position to sector
    mom_ratio = stock_mom_20 / (sector_mom_20.abs() + 1e-8)
    # Normalize to 0-1 range (sigmoid-like)
    df['feat_sector_rank'] = 1 / (1 + np.exp(-mom_ratio))

    # feat_sector_corr: Correlation with sector index
    stock_ret = temp_df['stock_ret']
    sector_ret = temp_df['sector_ret']
    df['feat_sector_corr'] = stock_ret.rolling(window=20).corr(sector_ret)

    # Fill NaN values
    df['feat_sector_relative_mom'] = df['feat_sector_relative_mom'].fillna(0.0)
    df['feat_sector_relative_vol'] = df['feat_sector_relative_vol'].fillna(0.0)
    df['feat_sector_rank'] = df['feat_sector_rank'].fillna(0.5)
    df['feat_sector_corr'] = df['feat_sector_corr'].fillna(0.0)

    return df


def extract_features_with_cache(df: pd.DataFrame, stock_code: Optional[str] = None) -> pd.DataFrame:
    """
    带缓存的特征提取函数
    ...
    Returns:
        提取了特征的DataFrame
    """
    from quant.infra.cache_utils import get_feature_cache

    cache = get_feature_cache()

    # 尝试从缓存加载
    if stock_code and cache.enabled:
        cached = cache.get(stock_code)
        if cached is not None:
            logger.debug(f"使用缓存特征: {stock_code}")
            return cached
    ...
    # Phase 9: 内存优化
    df = _optimize_feature_memory(df)
...
def batch_extract_features(
    ...
    Returns:
        {stock_code: features_df} 字典
        
    Example:
        stocks = {'sh.600000': df1, 'sh.600001': df2}
        features = batch_extract_features(stocks, n_jobs=4)
    """
    ...
    if use_cache:
        ...
        if cached_codes:
            logger.info(f"从缓存加载 {len(cached_codes)} 只股票特征")
    
    if not codes:
        return results
    
    # 并行处理
    logger.info(f"批量提取特征: {len(codes)} 只股票, {n_jobs} 并行作业")
    
    def process_single(args):
        code, df = args
        try:
            features = extract_features(df.copy())
            features = _optimize_feature_memory(features)
            ...
            logger.warning(f"特征提取失败 {code}: {e}")
            return code, None
    
    # 使用进程池并行处理
    items = [(code, stock_data_dict[code]) for code in codes]
    
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        futures = {executor.submit(process_single, item): item[0] for item in items}
        
        if show_progress:
            from tqdm import tqdm
            iterator = tqdm(as_completed(futures), total=len(futures), desc="提取特征")
        else:
            iterator = as_completed(futures)
        
        for future in iterator:
            code, features = future.result()
            if features is not None:
                results[code] = features
    
    logger.info(f"批量特征提取完成: {len(results)}/{len(stock_data_dict)} 只股票")
    return results
...
def get_feature_names() -> list:
    ...
    return [
        # 原始特征
        'feat_pct_chg_1', 'feat_pct_chg_3', 'feat_pct_chg_5', 'feat_pct_chg_10', 'feat_pct_chg_20',
        'feat_price_pctl_20',
        'feat_body_ratio', 'feat_lower_shadow_ratio', 'feat_upper_shadow_ratio',
        'feat_atr_ratio', 'feat_atr_mom_5',
        'feat_rsi_val', 'feat_rsi_diff',
        'feat_bias_s', 'feat_bias_l',
        'feat_macd_norm', 'feat_macd_diff',
        'feat_bb_pos', 'feat_bb_width',
        'feat_vol_pct_chg_1', 'feat_vol_pct_chg_5', 'feat_vol_ratio_5', 'feat_vol_slope',
        'feat_realized_vol_20', 'feat_vol_regime_chg',
        'feat_turnover',
        'feat_market_pct_1', 'feat_market_pct_5', 'feat_market_pct_10', 'feat_market_pct_20', 'feat_market_bias_20',
        'feat_rsi_rank_252', 'feat_mom_rank_252',
        # 新增特征
        'feat_mom_rank_100', 'feat_vol_rank_100', 'feat_rsi_rank_100',
        'feat_money_flow', 'feat_money_flow_ma_ratio', 'feat_money_flow_ma_ratio_zscore',
        'feat_money_flow_trend', 'feat_money_flow_zscore',
        'feat_vol_of_vol', 'feat_vol_skew', 'feat_vol_kurt',
        'feat_market_corr_20',
        'feat_sector_relative_mom', 'feat_sector_relative_vol', 'feat_sector_rank', 'feat_sector_corr',
        # 蜡烛图模式
        'feat_pattern_hammer', 'feat_pattern_shooting_star', 'feat_pattern_bullish_engulf',
        'feat_pattern_bearish_engulf', 'feat_pattern_doji', 'feat_pattern_morning_star',
        'feat_pattern_evening_star', 'feat_pattern_three_white',
    ]


def compute_all_features_for_stock(df: pd.DataFrame, stock_code: str,
                                    sector_code: Optional[str] = None) -> pd.DataFrame:
    """
    计算一只股票的所有特征（包括板块相对特征）

    Args:
        df: 股票数据DataFrame
        stock_code: 股票代码
        sector_code: 所属板块代码（可选）

    Returns:
        完整的特征DataFrame
    """
    # 基础特征
    df = extract_features_with_cache(df, stock_code)

    # 板块相对特征
    if sector_code:
        df = add_sector_relative_features(df, sector_code)
    else:
        # 如果没有指定板块，填充默认值
        df['feat_sector_relative_mom'] = 0.0
        df['feat_sector_relative_vol'] = 0.0
