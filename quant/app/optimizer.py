...
from quant.core.strategy_params import CORE_PARAM_SPACE, StrategyParams
...
# =============================================================================
# Parallel Walk-Forward Optimization (Phase 9 Performance Enhancement)
# =============================================================================
...
def _evaluate_fold(
    ...
    try:
        total_len = len(dates)
        pivot_ratio = 0.4 + (fold_idx * 0.2)  # 0.4, 0.6, 0.8
...
def walk_forward_cv(
    ...
    if parallel and n_folds > 1:
        # 并行执行 fold 评估
        logger.debug(f"使用并行 walk-forward: n_folds={n_folds}, n_jobs={n_jobs}")
        
        # 使用 joblib 进行并行计算
        fold_scores = Parallel(n_jobs=n_jobs, backend='loky')(
            delayed(_evaluate_fold)(i, codes, params, dates, objective_name)
            for i in range(n_folds)
        )
        
        # 过滤掉 None 值
        fold_test_scores = [s for s in fold_scores if s is not None]
    else:
        # 顺序执行
        for i in range(n_folds):
            score = _evaluate_fold(i, codes, params, dates, objective_name)
            if score is not None:
                fold_test_scores.append(score)
    
    if len(fold_test_scores) == 0:
        return 0.0  # No folds had any trades
    return float(np.mean(fold_test_scores))


def walk_forward_cv_parallel(
    codes: list[str],
    ...
    """
    并行 walk-forward 交叉验证的便捷函数
    
    Args:
        codes: 股票代码列表
        params: 策略参数，None 则使用默认参数
        n_folds: fold 数量
        n_jobs: 并行作业数，-1 表示使用所有 CPU
        objective_name: 目标函数名称
    ...
    Example:
        score = walk_forward_cv_parallel(['sh.600000'], None, n_folds=3, n_jobs=4)
    """
    if params is None:
        params = StrategyParams()
