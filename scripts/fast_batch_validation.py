...
通过随机选择股票样本，在 20+ 日期快速获取 100+ 交易样本
"""
import sys
...
def scan_and_test(test_date, stock_files, hold_days=5):
    """扫描并测试单个日期"""
    signals = []
...
def run_fast_validation(test_dates, hold_days=5, stocks_per_date=40):
    """运行快速验证"""
    data_path = Path(CONF.history_data.data_dir)
    all_stock_files = list(data_path.glob("*.csv"))
    all_stock_files = [f for f in all_stock_files if f.name not in ["stock-list.csv", "sh.000001.csv"]]
    
    all_results = []
    
    for test_date in test_dates:
        # 随机选择股票样本
        selected_stocks = random.sample(all_stock_files, min(stocks_per_date, len(all_stock_files)))
        results = scan_and_test(test_date, selected_stocks, hold_days)
    ...
    avg_pnl = df['pnl_pct'].mean() if total > 0 else 0
    median_pnl = df['pnl_pct'].median() if total > 0 else 0
    
    gross_profit = wins['pnl_pct'].sum() if len(wins) > 0 else 0
    gross_loss = abs(losses['pnl_pct'].sum()) if len(losses) > 0 else 1
    profit_factor = gross_profit / gross_loss
    
    avg_max_profit = df['max_profit_pct'].mean() if total > 0 else 0
    avg_max_dd = df['max_drawdown_pct'].mean() if total > 0 else 0
    ...
    for tier in df['ai_tier'].dropna().unique():
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("快速批量验证 - 20+ 历史日期随机采样")
    print("=" * 80)
    print(f"测试日期数：{len(test_dates)}")
    print(f"测试日期范围：{test_dates[0]} 至 {test_dates[-1]}")
    print(f"持股天数：5 天（带止损止盈）")
    print(f"扫描范围：每日期随机 40 只股票")
    print(f"预期交易数：100+ 笔")
    print("=" * 80)
    
    # 执行快速验证
    results_df = run_fast_validation(test_dates, hold_days=5, stocks_per_date=40)
